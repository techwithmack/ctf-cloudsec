...
`challenge-2-iac/bootstrap/`, and the image is built/pushed exactly once here, before the first
team deploys.
...
`challenge-2-iac/` reuses the same `:latest` image. The image itself is generic — all per-team
